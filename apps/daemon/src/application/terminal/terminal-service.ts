...
export class TerminalService {
  ...
  create(projectId: string, type: TerminalSessionType, cwd: string): TerminalSession {
    ...
    this.terminals.set(id, { session, child: null, ring: [], seq: 0 });
  ...
  /** Writes a line to the shell's stdin. Returns false for non-running sessions. */
  write(id: string, data: string): boolean {
    const m = this.terminals.get(id);
    if (!m || !m.child?.stdin?.writable || !["RUNNING", "WAITING"].includes(m.session.status)) return false;
    m.child.stdin.write(data.endsWith("\n") ? data : `${data}\n`);
