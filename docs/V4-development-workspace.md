...
| file search (이름 기준) | ✅ | /files/search?q= — 내용 grep은 📋 후속 |
...
| changed-by 어느 agent 연결 | 🟡 | agent.run_output/task 이벤트로 간접 추적; 파일→run 직접 매핑 미구현 |
| direct editing (Safe Edit Guard 연계) | 📋 | 아키텍처는 준비(path-guard/lease), UI 편집기 미구현 |
...
| 세션 타입 USER/AGENT/TEST/BUILD 구분 | ✅ entity / 🟡 UX(탭 분리는 USER만 구현) |
| 완전 TTY 인터랙션(vi/cursor 제어) | 📋 | piped stdio 기반 — node-pty 도입 시 전환점 문서화됨 |
...
| RUNTIME_CHANGE/PAUSE/CANCEL 등 structured command | 🟡 | 분류까지 ✅, RUNTIME_CHANGE의 override 자동 적용은 📋 (UI에서 수동 가능) |
| Lead 응답(LLM tier + deterministic fallback) | ✅ | provider 있으면 generate, mock이면 상태 요약 응답 |

## 5. Realtime interface

- WS `/ws`: project events(task.*, agent.run_output, workflow.*) + terminal.output/status broadcast. ✅
- file.changed/git.changed 푸시는 fs watcher 미구현으로 📋 — 현재는 workspace API 재호출로 갱신.
